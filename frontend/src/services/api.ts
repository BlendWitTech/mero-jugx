...
api.interceptors.request.use(
  (config: InternalAxiosRequestConfig) => {
    ...
    const isBoardsRequest = url.includes('/boards/');
    const isInAppContext = pathname.includes('/apps/');
...
api.interceptors.response.use(
  ...
  async (error: AxiosError) => {
    ...
    if (error.response?.status && error.response.status !== 401 && !isAuthEndpoint && !isMfaSetupEndpoint) {
      ...
      // These occur when user doesn't have access to mero-board app (handled gracefully in UI)
