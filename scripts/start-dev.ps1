...
Write-Host "[1/2] Starting backend server (port 3000)..." -ForegroundColor Blue
...
Write-Host "[2/2] Starting frontend server (port 3001)..." -ForegroundColor Blue
...
try {
    while ($true) {
        ...
        if ($backendWindow.HasExited -or $frontendWindow.HasExited) {
            Write-Host ""
