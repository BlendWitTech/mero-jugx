...
export class OrganizationAppsService {
  ...
  async purchaseApp(
    ...
    // Grant access to organization owner automatically
    ...
    if (dto.user_ids && dto.user_ids.length > 0) {
      for (const targetUserId of dto.user_ids) {
        // Skip if it's the owner (already granted above)
        if (targetUserId === ownerMember?.user_id) {
          continue;
        }
  ...
  async updateSubscription(
    ...
    if (dto.auto_renew !== undefined) {
      ...
      if (!wasAutoRenew && dto.auto_renew) {
        await this.sendAutoRenewalActivatedEmail(orgId, subscription);
      }
    }
