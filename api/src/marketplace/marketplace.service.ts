...
export class MarketplaceService {
  ...
  async getLastUsed(userId: string, organizationId: string) {
    ...
    // Remove duplicates and return unique apps, excluding system-admin
    const uniqueApps = new Map<number, App>();
    records.forEach((r) => {
      if (r.app && !uniqueApps.has(r.app.id) && r.app.slug !== 'system-admin') {
  ...
  async getFavorites(userId: string, organizationId: string) {
    ...
    // Filter out system-admin app
    return favs.map((f) => f.app).filter((app) => app && app.slug !== 'system-admin');
  }
  ...
  async getPinned(userId: string, organizationId: string) {
    ...
    // Filter out system-admin app
    return pinned.map((p) => p.app).filter((app) => app && app !== null && app.slug !== 'system-admin');
  }
