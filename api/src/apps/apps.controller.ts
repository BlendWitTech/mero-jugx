...
export class AppsController {
  ...
  @Get('slug/:slug')
  @Permissions('apps.view')
  @ApiOperation({ summary: 'Get app by slug' })
  ...
  async getAppBySlug(@Param('slug') slug: string) {
    return this.appsService.findOneBySlug(slug);
  }
...
export class MarketplaceController {
  ...
  @Get('slug/:slug')
  @Public()
  @ApiOperation({ summary: 'Get app details by slug (Public - no auth required)' })
  @ApiParam({ name: 'slug', description: 'App slug' })
  @ApiResponse({ status: 200, description: 'App retrieved successfully' })
  @ApiResponse({ status: 404, description: 'App not found or not available' })
  async getAppDetailsBySlug(@Param('slug') slug: string) {
    // System-admin should not be accessible through marketplace
    if (slug === 'system-admin') {
      throw new NotFoundException('App not found or not available');
    }
    const app = await this.appsService.findOneBySlug(slug);
    return this.appsService.getPublicAppDetails(app.id);
  }
