...
interface WorkspaceTemplateSelectorProps {
  onTemplateUsed?: (workspaceId: string) => void;
  onCancel?: () => void;
}

export default function WorkspaceTemplateSelector({
  onTemplateUsed,
  onCancel,
  ...
  const { data: templates = [], isLoading } = useQuery<WorkspaceTemplate[]>({
    queryKey: ['workspace-templates', appSlug],
    queryFn: async () => {
      const response = await api.get(`/apps/${appSlug}/workspace-templates`);
  ...
  const useTemplateMutation = useMutation({
    mutationFn: async (templateId: string) => {
      const response = await api.post(`/apps/${appSlug}/workspace-templates/use`, {
        template_id: templateId,
        workspace_name: customWorkspaceName || undefined,
      });
      return response.data;
    },
    onSuccess: (workspace) => {
      queryClient.invalidateQueries({ queryKey: ['workspaces', appSlug] });
      queryClient.invalidateQueries({ queryKey: ['workspace-templates', appSlug] });
      toast.success('Workspace created from template successfully');
      onTemplateUsed?.(workspace.id);
      setSelectedTemplate(null);
      setCustomWorkspaceName('');
    },
    onError: (error: any) => {
      toast.error(error.response?.data?.message || 'Failed to create workspace from template');
    },
  });
  ...
  const getCategoryIcon = (category: string) => {
    const iconMap: Record<string, React.ReactNode> = {
      basic: <LayoutDashboard className="h-5 w-5" />,
      scrum: <Zap className="h-5 w-5" />,
      kanban: <BarChart3 className="h-5 w-5" />,
      agile: <Rocket className="h-5 w-5" />,
      marketing: <TrendingUp className="h-5 w-5" />,
      product: <Target className="h-5 w-5" />,
      agency: <Briefcase className="h-5 w-5" />,
    };
    return iconMap[category] || <FolderKanban className="h-5 w-5" />;
  };
    ...
    <div className="space-y-4">
      <div className="mb-2">
        ...
        <p className="text-sm" style={{ color: theme.colors.textSecondary }}>
          Select a template to quickly set up your workspace with pre-configured projects
        </p>
      </div>
      ...
      {templates.length === 0 ? (
        <EmptyState
          ...
          description="Create a template from an existing workspace or start from scratch"
        />
      ) : (
                  ...
                  <div className="flex items-center justify-between pt-3 border-t" style={{ borderColor: theme.colors.border }}>
                    <div className="flex items-center gap-4 text-xs" style={{ color: theme.colors.textSecondary }}>
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="font-medium">{template.projects?.length || 0} projects</span>
                      </div>
                      <div className="flex items-center gap-1.5">
        ...
        <div className="space-y-5">
            ...
            <div className="p-4 rounded-lg border" style={{
              ...
              <div className="flex items-center gap-3">
                <div
                  ...
                  style={{
                    backgroundColor: getCategoryColor(templates.find(t => t.id === selectedTemplate)?.category || 'basic') + '15',
                    color: getCategoryColor(templates.find(t => t.id === selectedTemplate)?.category || 'basic')
                  }}
                >
                  {getCategoryIcon(templates.find(t => t.id === selectedTemplate)?.category || 'basic')}
                </div>
          ...
          <div>
            <label className="block text-sm font-medium mb-2" style={{ color: theme.colors.text }}>
              Workspace Name <span className="text-xs font-normal" style={{ color: theme.colors.textSecondary }}>(optional)</span>
            </label>
            <Input
            ...
            <p className="text-xs mt-2" style={{ color: theme.colors.textSecondary }}>
              If left empty, the workspace will use the template name
            </p>
          </div>
